...
const EnhancedTableToolbar = (props) => {
  ...
  return (
    <Toolbar
      className={clsx(classes.root, 'text-white', 'd-flex', 'justify-content-center', 'text-center', "dwTitle", 'pt-4')}>
      <h6 className="line-height-1 font-weight-bold pt-1">
        {getString(defiStrings.string37, selectedLangauge) + ' for $DRIP'}
      </h6>
...
export default function DripTransactionsTable(props) {
  ...
  const { selectedLangauge, transactionFilter, bnbPrice, subscription } = useStoreState((state) => state.Dapp);
  ...
  const generateRows = (transactions, currentWallets, selectedLangauge) => {
    ...
    keys.forEach((key, index) => {
      ...
      let newTrade = createData(trades[0].tranId, trades[0].buySell, 0, 0, 0, trades[0].wallet, 999999999999, trades[0].actualTime, trades[0].walletType, trades[0].isWalletSaved)
  ...
  const EnhancedTableFooterComponent = () => <EnhancedTableFooter numSelected={selected.length} saveWallets={handleSavedWallets} removeWallets={handleRemoveWallets} selectedWallets={selectedWallets} />
  return (
    <>
      <Card className="bg-first px-2 text-center" >
        <CSVLink
          data={tableData}
          filename="data.csv"
          className='hidden'
        ...
        {isLoading || noData || isLoading === undefined ?
          ...
          : (
            <>
              <TableContainer className="tableHeight-topBuyers-drip">
                <Table
                  stickyheader="true"
                    ...
                    {stableSort(displayRows, getComparator(order, orderBy))
                      ...
                      .map((row, index) => {
                        ...
                        if ((subscription.level < 1 && index > 4) || (page > 0 && subscription.level < 1)) {
                          row = rowScramble(row, 'dripOverview');
                        }
                        return (
                          <TableRow
        ...
        {displayRows.length > 0 && !isLoading ? (
          <TablePagination
            ActionsComponent={EnhancedTableFooterComponent}
