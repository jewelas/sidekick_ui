...
import { TableBody, Table, TableCell, TableContainer, TableHead, TableRow, Toolbar, IconButton, Tooltip, Grid } from '@material-ui/core';
...
function EnhancedTableHead(props) {
  ...
  const headCells = [
    ...
    { id: 'transactions', numeric: false, disablePadding: false, label: getString(defiStrings.TableLabels.string3, selectedLangauge) },
...
const EnhancedTableFooter = (props) => {
  ...
  const { numSelected, selectedWallets, count, page, rowsPerPage, onPageChange } = props;
  const { selectedLangauge } = useStoreState((state) => state.Dapp);
  const saveWallets = (e) => {
...
const EnhancedTableToolbar = (props) => {
  ...
  return (
    <Toolbar
      className={clsx(classes.root, 'text-white', 'd-flex', 'justify-content-center', 'text-center', "dwTitle", "pt-3")}>
      <h6 className="line-height-1 font-weight-bold">
        {tokenSymbol !== undefined && tokenSymbol.length > 0 ? getString(defiStrings.string37, selectedLangauge) + ' for $' + tokenSymbol : getString(defiStrings.string37, selectedLangauge)}
      </h6>
...
function buildRow(row, currentWallets, buyString, sellString) {
  ...
  return createData(transaction.hash, buyOrSell, buySellAmount, 1, baseAmount, transaction.txFrom.address, 0, 'WHALE', block.timestamp.time, "", isWalletSaved);
}
...
function generateRows(transactions, currentWallets, selectedLangauge) {
  ...
  let groupedTrades = groupBy(rows, (trade) => {
    return trade.wallet + '|' + trade.buySell;
  });
...
export default function TransactionTable(props) {
  ...
  const { selectedLangauge, transactionFilter, subscription } = useStoreState((state) => state.Dapp);
  ...
  const handleOpenBSC = (event, row) => {
    ...
    window.open(bsc + row.wallet, '_blank');
  }
  ...
  const tokenSymbol = transactionFilter !== null && transactionFilter.token !== null && transactionFilter.token !== undefined ? transactionFilter.token.subject.symbol : '';
  const EnhancedTableFooterComponent = (props) => <EnhancedTableFooter numSelected={selected.length} saveWallets={handleSavedWallets} removeWallets={handleRemoveWallets} selectedWallets={selectedWallets} />

  return (
    <>
      <Card className="bg-first px-2 pt-3 text-center mb-1" style={{ minHeight: '44vh' }}>
        <CSVLink
          data={tableData}
          filename="data.csv"
          className='hidden'
        ...
        {isLoading || noData || isLoading == undefined ?
          ...
          : (
            <>
              <TableContainer className="tableHeight-topBuyers">
                <Table
                  stickyheader="true"
                      ...
                      stableSort(displayRows, getComparator(order, orderBy))
                        ...
                        .map((row, index) => {
                          ...
                          if ((subscription.level < 1 && index > 4) || (page > 0 && subscription.level < 1)) {
                            ...
                            row = rowScramble(row, 'defiOverview');
                          }
                          ...
                          return (
                            <TableRow
                              ...
                              hover
                              onClick={subscription.level < 1 && index > 4 ? () => handleBlurredClick() : (event) => handleClick(event, row.tranId)}
                              aria-checked={isItemSelected}
                              tabIndex={-1}
                              key={row.tranId + row.wallet}
                              selected={isItemSelected}>
                              <TableCell align="left" width="1%">{rank}</TableCell>
                              <TableCell align="left" width="0%" className='tx-table-avatar'><AvatarCheck address={row.wallet} size={`xs`} /></TableCell>
                              <TableCell align="left" width="30%" className="cellWallet"> <NameServiceComponent address={row.wallet} /></TableCell>
                              <TableCell align="center" width="5%">{row.buySell.toLowerCase() === getString(defiStrings.string8, selectedLangauge).toLowerCase() ? (
                              ...
                              <TableCell align="center" width="5%">{row.transactions}</TableCell>
                              ...
                              <TableCell align="center" width="5%" className="skSavedIcon">{row.isWalletSaved && (<SaveIcon></SaveIcon>)}</TableCell>
        ...
        {displayRows.length > 0 && !isLoading ? (
          <TablePagination
            // ActionsComponent={EnhancedTableFooterComponent} ##todo fix this w/ the arrow left and right working too
