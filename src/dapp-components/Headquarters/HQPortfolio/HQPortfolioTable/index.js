...
function buildRow(row) {
  const { address, totalUSD, totalTokens, price, symbol, } = row;
  // take symbol get price, check user wallet for tokens
  return createData(symbol, address, price, totalUSD, totalTokens);
}
...
export default function HQPortfolioTable(props) {
  ...
  return (
    <>
      <Card className="bg-first text-center pt-1">
        {isLoading || noData || isLoading == undefined ?
          (
            <div>
              {props.noData ?
               (<h5 className="text-white pt-6 mb-8">{currentUserAddress === undefined ? 'Please connect wallet to view portfolio' :
          ...
          : (
            <>
              <TableContainer className="tableHeight-portfolio">
                <Table
                  stickyheader="true"
                      ...
                      .map((row, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <TableCell align="right" width="10%">{row.price == undefined ? 0 : row.price < .001 ?
                              "$" + row.price.toLocaleString(undefined, { 'minimumFractionDigits': 2, 'maximumFractionDigits': 4 })
                              : "$" + row.price.toLocaleString(undefined, { 'minimumFractionDigits': 2, 'maximumFractionDigits': 2 })}</TableCell>
                            <TableCell align="right" width="20%">{"$" + row.totalUSD.toLocaleString(undefined, { 'minimumFractionDigits': 2, 'maximumFractionDigits': 2 })}</TableCell>
                            <TableCell align="right" width="20%">{row.totalTokens > 100 ? row.totalTokens.toLocaleString(undefined, { 'minimumFractionDigits': 0, 'maximumFractionDigits': 0 })
                              : row.totalTokens.toLocaleString(undefined, { 'minimumFractionDigits': 3, 'maximumFractionDigits': 3 })}</TableCell>
                            <TableCell align="center" width="20%" className="skSavedIcon">
                              {row.address === 'bnb' ? undefined : (
                                <>
